...
class TestCostTracker:
    ...
    def test_record_multiple_models(self, tracker):
        tracker.record("deepseek-chat", "deepseek", "writer", "generate",
                        "test_book", 1, 300, 150, latency_ms=800)
        tracker.record("gpt-4o", "openai", "editor", "polish",
                        "test_book", 2, 400, 250, latency_ms=2000)
        tracker.record("deepseek-chat", "deepseek", "writer", "generate",
                        "test_book", 3, 200, 100, latency_ms=500)
        status = tracker.get_budget_status()
        assert status.total_calls == 3

    def test_get_summary(self, tracker):
        tracker.record("deepseek-chat", "deepseek", "writer", "generate",
                        "test_book", 1, 1000, 500, latency_ms=1500)
    ...
    def test_report_generation(self, tracker):
        tracker.record("deepseek-chat", "deepseek", "writer", "generate",
                        "test_book", 1, 200, 100, latency_ms=500)

...
class HumanizeEngine:
    ...
    def __init__(
        ...
        self.strategies = strategies or list(HumanizeStrategy)
    ...
    def _replace_high_risk_words(self, text: str):
        ...
        for ai_word, replacements in HIGH_RISK_REPLACEMENTS.items():
            if ai_word not in text:
                continue
            replace_prob = 0.5 + self.aggressive * 0.5
    ...
    def _remove_redundant_modifiers(self, text: str):
        ...
        for pattern, replacement in redundant_patterns:
            ...
            if matches:
                remove_count = int(len(matches) * (0.3 + self.aggressive * 0.5))
    ...
    def _vary_sentence_length(self, text: str):
        ...
        result = []
        for s in sentences:
            if len(s) > 60 and self.aggressive > 0.3:
                parts = s.split("，")
                if len(parts) >= 3:
                    mid = len(parts) // 2
    ...
    def _add_human_phrases(self, text: str):
        ...
        add_count = int(len(matches) * self.aggressive * 0.3)
        phrases = ["卧槽", "离谱", "绝了", "这波", "舒服了", "稳了", "麻了"]
    ...
    def _vary_punctuation(self, text: str):
        ...
        if not periods:
            return text, count
        change_count = int(len(periods) * self.aggressive * 0.1)
    ...
    def _split_long_paragraphs(self, text: str):
        ...
        result = []
        for p in paragraphs:
            if len(p) > 150 and self.aggressive > 0.4:

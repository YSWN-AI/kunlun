...
def _book_dir(book_id: str) -> str:
    path = _os.path.join("data", "books", f"book_{book_id}")
    _os.makedirs(path, exist_ok=True)
    return path


def _load_json(book_id: str, filename: str, default):
    path = _os.path.join(_book_dir(book_id), filename)
...
async def get_outline(book_id: str = "default") -> dict:
    try:
        ...
        return {"success": True, "nodes": data.get("nodes", []), "foreshadows": data.get("foreshadows", [])}
    except Exception as e:
            ...
            for item in items:
                if item.get("id") == item_id:
                    ...
                    if "section" in update_data and update_data["section"] != sec_name:
                        items.remove(item)
                        data.setdefault("sections", {}).setdefault(update_data["section"], []).append(item)
                    break
            if found:
                break
        if not found:

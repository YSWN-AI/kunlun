...
API 参考: https://b.faloo.com/ (作者后台)
需要配置: FEILU_COOKIE / FEILU_TOKEN

特性:
- 章节字数: 1500-20000
- 支持: 同人/二次元/玄幻/都市/言情/科幻/游戏
- 同人创作: 主要特色
- VIP 章节: 支持
"""
...
class FeiluAdapter(BasePlatformAdapter):
    """飞卢小说网适配器"""

    PLATFORM_ID = "feilu"
    PLATFORM_NAME = "飞卢小说网"
    PLATFORM_URL = "https://b.faloo.com"
    API_BASE = "https://api.feilu.com/api"
    ...
    async def publish(self, chapter: ChapterData) -> PublishResult:
        ...
        if not self._cookie:
            return PublishResult(
                ...
                platform_id=self.PLATFORM_ID,
                platform_name=self.PLATFORM_NAME,
                message="未配置飞卢小说网 Cookie，请设置 FEILU_COOKIE 环境变量",
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data.get("code") == 0:
                    return PublishResult(
                        ...
                        message=f"章节「{chapter.title}」已提交到飞卢小说网，等待审核",
                    )
                return PublishResult(
                    success=False,
                    platform_id=self.PLATFORM_ID,
            ...
            if resp.status_code == 401:
                return PublishResult(
                    ...
                    platform_id=self.PLATFORM_ID,
                    platform_name=self.PLATFORM_NAME,
                    message="Cookie 已过期，请重新登录飞卢小说网作者后台",
                    status=PublishStatus.FAILED,
                    error="AUTH_EXPIRED",
                )
            return PublishResult(
    ...
    async def validate_chapter(self, chapter: ChapterData) -> list[str]:
        ...
        if chapter.genre and chapter.genre not in self.SUPPORTED_GENRES:
            issues.append(f"题材 '{chapter.genre}' 不在飞卢小说网支持的分类中")
        return issues

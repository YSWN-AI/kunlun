...
API 参考: https://writer.zongheng.com/ (作家专区)
需要配置: ZONGHENG_COOKIE / ZONGHENG_TOKEN

特性:
- 章节字数: 1000-20000
- 支持: 玄幻/奇幻/武侠/仙侠/都市/历史/军事/游戏/体育/科幻/悬疑
- 买断+分成双模式
- 定时发布: 支持
"""
...
class ZonghengAdapter(BasePlatformAdapter):
    """纵横中文网适配器"""

    PLATFORM_ID = "zongheng"
    PLATFORM_NAME = "纵横中文网"
    PLATFORM_URL = "https://www.zongheng.com"
    API_BASE = "https://writer.zongheng.com/api"
    ...
    async def publish(self, chapter: ChapterData) -> PublishResult:
        ...
        if not self._cookie:
            return PublishResult(
                ...
                platform_id=self.PLATFORM_ID,
                platform_name=self.PLATFORM_NAME,
                message="未配置纵横中文网 Cookie，请设置 ZONGHENG_COOKIE 环境变量",
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if data.get("code") == 0:
                    return PublishResult(
                        ...
                        message=f"章节「{chapter.title}」已提交到纵横中文网，等待审核",
                    )
                return PublishResult(
                    success=False,
                    platform_id=self.PLATFORM_ID,
            ...
            if resp.status_code == 401:
                return PublishResult(
                    ...
                    platform_id=self.PLATFORM_ID,
                    platform_name=self.PLATFORM_NAME,
                    message="Cookie 已过期，请重新登录纵横中文网作家专区",
                    status=PublishStatus.FAILED,
                    error="AUTH_EXPIRED",
                )
            return PublishResult(
    ...
    async def validate_chapter(self, chapter: ChapterData) -> list[str]:
        ...
        if chapter.genre and chapter.genre not in self.SUPPORTED_GENRES:
            issues.append(f"题材 '{chapter.genre}' 不在纵横中文网支持的分类中")
        return issues
